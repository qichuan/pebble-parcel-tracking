...
| `tools/` | Node tests, plus the courier-list generator. Not shipped.          |
...
npm run gen:couriers  # regenerate docs/couriers.js from data/ship24-couriers.csv
```
...
- Ship24 flow is **tracker-based** (the per-shipment plans): `POST /trackers`
  ...
  A saved parcel keeps its `trackerId` (and so skips re-registration) only
  while its `courierCode` is unchanged too: registration is the only thing
  that carries the courier to Ship24, so an edited courier has to re-register.
...
No build step, ES5-only (old Pebble webviews), and only one dependency — the
generated, committed `couriers.js` beside it. Opened by
...
The courier field is a type-to-filter picker, not a raw code box. Its data is
`docs/couriers.js` — every non-deprecated Ship24 courier packed as `name|code`
lines (plus `|ascii` where the name has accents, so "ceska" finds "Česká
pošta"), generated by `tools/gen-couriers.js` from `tools/data/ship24-couriers.csv`.
Refresh it by replacing that CSV and running `npm run gen:couriers`; Ship24's
`GET /public/v1/couriers` can't be called from the page, since the page never
receives the API key. The **visible** input holds the courier's name and the
hidden `.courier` input holds the code that gets saved. An unlisted but
code-shaped entry is still saved as typed, so hand-entered and deprecated codes
keep working; anything else blocks the save. If `couriers.js` fails to load the
field degrades to the plain code box it used to be — `tools/test-settings.js`
inlines the script (jsdom won't fetch it) and covers both paths.
